...
def submit_batch():
    ...
    try:
        with console.status("Submitting batch...", spinner="dots"):
...
def check_status(batch_id):
    ...
    client = anthropic.Anthropic()
    try:
        with console.status("Checking batch status...", spinner="dots"):
...
def retrieve_results(batch_id):
    ...
    try:
        with console.status("Retrieving results...", spinner="dots"):
