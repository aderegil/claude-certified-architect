...
Build a developer productivity agent that helps engineers explore an unfamiliar Python codebase using the Claude Agent SDK. You start with built-in tools (Grep, Glob, Read), add an MCP documentation server with enhanced descriptions, delegate deep exploration to a defined `AgentDefinition` subagent, persist findings to a scratchpad file across questions, and package the result as a Claude Code skill for team reuse. By the end, every exam concept traces to working code you can reference.
...
- **`run_query()`** sends the user's question to the Claude Agent SDK via `query()`. The SDK runs the agentic loop internally — it decides which tools to call, executes them, and continues until the answer is ready. There is no manual `while stop_reason == "tool_use"` loop.
- **`allowed_tools`** controls what the agent can use. The starter gives it five built-in tools: `Read`, `Write`, `Edit`, `Grep`, `Glob`. In later steps you will add MCP tools and the `Agent` tool for subagent spawning.
- **`system_prompt.txt`** tells the agent how to select the right tool for each task and how to explore incrementally. Open `prompts/system_prompt.txt` and read the "Tool selection" and "Exploration strategy" sections.
- **`PreToolUse` / `PostToolUse` hooks** intercept every tool call before and after execution. They run in your process, not inside the agent's context, so they print `→ Grep("...")` and `→ Read (app.py)` in real time as each tool call happens. This is the same pattern used for audit logs and compliance checks in production.
- **Each menu query creates a fresh agent session** — there is no memory between questions. The `<scratchpad>` section in the system prompt is empty for now. You will fix this in Step 6.
...
**What to observe:** The agent uses `Glob` (e.g., `**/test_*.py`) to find test files by naming pattern — not `Grep` or `Read`. `Glob` is the correct tool for "find files matching a pattern." The hook output shows `→ Glob("**/test_*.py")` or similar.

#### 3.2 Find callers (Grep)

Type `2`. The agent needs to find where a function is called across the codebase.

**What to observe:** The agent uses `Grep` (e.g., `validate_email`) to search file contents — not `Glob`. `Grep` is the correct tool for "find where this pattern appears in code." The hook output shows `→ Grep("validate_email")`.
...
**What to observe:** `middleware.py` has the same pattern string `r'^Bearer [A-Za-z0-9\-._~+/]+=*$'` in two functions. When the agent tries to use `Edit`, the anchor text matches both locations — `Edit` fails because the text is not unique. The agent falls back to `Read` + `Write` to make the change reliably. [Task 2.5]

> **Note:** The agent may sometimes succeed with `Edit` if it uses a larger anchor (including surrounding unique code). The key exam concept is knowing that `Edit` requires unique anchor text and that `Read` + `Write` is the fallback when it fails.
...
**What to observe:** The agent uses `Grep` and `Read` to search for clues in the source code, but the answer is incomplete — known technical debt items (priorities, planned improvements, root causes) are documented in project documentation, not in source code. The agent has no way to access that documentation yet. You will fix this in Step 4.
...
The lab includes a `lookup_docs` MCP tool in `tools.py` that searches project documentation (architecture notes, API specs, onboarding guides, known tech debt). But three things prevent the agent from using it:
1. The tool is not wired up in `main.py`
2. Its description is just `"Search documentation."` — too vague
3. The system prompt's tool selection section only mentions built-in tools, biasing the agent toward `Grep`
...
Tool descriptions are the primary mechanism the agent uses for tool selection — but they are not sufficient on their own. The system prompt's wording also affects tool selection: when the prompt says "use Grep for content search," it biases the agent toward `Grep` even when an MCP tool has a better description. Both the tool description AND the system prompt guidance need to work together for reliable tool selection. [Tasks 2.1, 2.4]
...
> **Also know for the exam:** In production, MCP servers are configured in `.mcp.json` (project-level, version controlled) or `~/.claude.json` (user-level, personal):
> ```json
> {
>   "mcpServers": {
>     "docs": {
...
> }
> ```
> The `${DOCS_API_KEY}` syntax expands environment variables at connection time — credentials never appear in the config file. Project-level `.mcp.json` is shared via version control for team tooling; user-level `~/.claude.json` is for personal or experimental servers. This lab uses `create_sdk_mcp_server` (in-process) for simplicity, but the exam tests `.mcp.json` configuration. [Task 2.4]

> **Also know for the exam:** MCP resources expose content catalogs (e.g., documentation hierarchies, database schemas) so agents see available data without exploratory tool calls. Community MCP servers should be preferred over custom implementations for standard integrations (e.g., Jira, GitHub) — reserve custom servers for team-specific workflows. [Task 2.4]

### Step 5 — Define the Explore subagent

#### 5.1 The problem

All exploration currently happens in the main agent's context window. For the simple queries in Step 3, this is fine — `Grep` and `Read` answer them in a few calls. But in production codebases with hundreds of files, complex questions (dependency tracing, architecture discovery) generate verbose tool output that fills the main context and crowds out the actual answer. The exam tests the pattern for solving this: delegating deep investigation to an isolated subagent.
...
- The subagent's tool calls (`Grep`, `Read` across multiple files) appear indented as `[explore]` messages — this output stays in the subagent's isolated context, not the main agent's
- The main agent receives a structured summary from the subagent and uses it to answer
- The main agent's context stays clean — it only sees the summary, not every `Grep` and `Read` the subagent performed

> **Note:** At this codebase size the agent may sometimes answer directly with `Grep` without spawning the subagent — the Explore subagent's value is architectural. In production codebases, the isolation becomes essential for keeping the main agent's context manageable.

**Key detail:** The main agent includes relevant context in the subagent's prompt. The subagent does NOT inherit the main agent's conversation history — it starts with only the prompt the main agent writes for it. This is _explicit context passing_: the main agent must include everything the subagent needs to know. [Task 1.3]

#### 5.4 Exam concept

`AgentDefinition` configures a subagent with a description (when to use it), a system prompt (how it should behave), and `tools` (what it can access). Restricting `tools=["Read", "Grep", "Glob"]` prevents the subagent from writing files or spawning sub-subagents — it can only explore. The subagent isolates verbose discovery output from the main agent's context, serving the same purpose as plan mode's discovery phase in Claude Code. [Tasks 1.3, 2.3, 3.4, 5.4]

> **Also know for the exam:** Plan mode is designed for large-scale changes (multi-file edits, architectural decisions) and enables safe exploration before committing to changes. Direct execution is appropriate for simple, well-scoped changes (single-file bug fix with a clear stack trace). The Explore subagent pattern is the Agent SDK equivalent of plan mode's discovery phase — it isolates verbose investigation while the main agent preserves high-level coordination. Use plan mode when working in Claude Code directly; use Explore subagents when building programmatic agents. [Task 3.4]

> **Also know for the exam:** In production, the main agent would _summarize key findings from one exploration phase before spawning sub-agents for the next phase_, injecting summaries into initial context. The coordinator prompt should specify research goals and quality criteria, not step-by-step procedures, to enable subagent adaptability. [Task 1.3]
...
Each call to `run_query()` creates a new agent session with a fresh context. The system prompt includes a `<scratchpad>` section, and the agent may write findings to `scratch.md` after the first question. But the scratchpad content is never loaded back — the second question starts with `"No previous findings."` and the agent must re-explore to answer.
...
Context degradation in extended sessions causes agents to give inconsistent answers and reference "typical patterns" rather than specific classes they discovered earlier. Scratchpad files persist key findings across context boundaries — the agent writes what it found, and the scratchpad is loaded into the system prompt for subsequent questions. This counteracts context degradation by keeping critical facts available even when earlier tool results have scrolled out of context. [Task 5.4]

> **Also know for the exam:** Use `/compact` to reduce context usage during extended exploration sessions when context fills with verbose discovery output. For crash recovery in production, each agent should export state to a known location (a manifest file), and the coordinator loads the manifest on resume and injects it into agent prompts. [Task 5.4]
...
This imports the coding conventions from a separate file instead of inlining them in `CLAUDE.md`. The `@import` syntax keeps `CLAUDE.md` modular — you can maintain conventions, testing standards, and API guidelines in separate files and import only what is relevant.

Open `.claude/conventions.md` to see the imported content. When Claude Code loads this lab's `CLAUDE.md`, it resolves the `@import` and applies the conventions automatically.

**Exam concept:** `CLAUDE.md` supports a three-level hierarchy: user-level (`~/.claude/CLAUDE.md`), project-level (`.claude/CLAUDE.md` or root `CLAUDE.md`), and directory-level (subdirectory `CLAUDE.md` files). User-level settings are not shared via version control — if a team member's instructions are only at user-level, new teammates will not receive them. Use `@import` to keep `CLAUDE.md` modular and `.claude/rules/` for topic-specific rule files as an alternative to a monolithic `CLAUDE.md`. [Task 3.1]

#### 7.2 Complete the /explore skill

Open `.claude/commands/explore.md`. This is a Claude Code skill that runs codebase exploration — the same kind of investigation the Python agent does, but invokable from a Claude Code session via `/explore`.

The frontmatter is incomplete. Add the following fields inside the `---` block:
...
Project-scoped skills in `.claude/commands/` are shared via version control — every team member gets the `/explore` command. `context:fork` isolates verbose output (codebase analysis, brainstorming) from the main conversation. `allowed-tools` restricts tool access during skill execution. `argument-hint` prompts for required parameters. Choose skills for on-demand task-specific workflows; use `CLAUDE.md` for always-loaded universal standards. [Task 3.2]

> **Also know for the exam:** Personal skills in `~/.claude/commands/` or `~/.claude/skills/` are for individual customization — they are not shared with teammates. If a team needs a standard workflow, put it in project-scoped `.claude/commands/`. If a developer wants a personal variant, they create a personal copy with a different name to avoid affecting teammates. [Task 3.2]
