...
Build a developer productivity agent that explores an unfamiliar Python codebase using the Claude Agent SDK. You start with built-in tools (`Grep`, `Glob`, `Read`), add an MCP documentation server, delegate deep exploration to an `AgentDefinition` subagent, persist findings to a scratchpad file, and package the result as a Claude Code skill. By the end you will have a working agent that understands a codebase, remembers what it discovers, and delegates deep investigation to isolated subagents. All grounded in exam concepts you can trace back to specific task statements.

![Lab 04 screenshot](_manage/screenshot.png)
...
Restores all modified files to their original starter state with TODOs intact and deletes `scratch.md`. Your `.env` file is preserved — no need to re-enter your API key.
