...
![Lab 01 screenshot](_manage/screenshot.png)

> **Note on SDK choice:** This lab uses the Claude API (Client SDK) with a manual agentic loop — not the Claude Agent SDK. This is intentional: the manual loop makes `stop_reason == "tool_use"` → execute → continue → `"end_turn"` → stop visible and concrete. Labs 03 and 04 use the Agent SDK where `query()` handles the loop internally. The exam tests both patterns.
